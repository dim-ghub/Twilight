...
setup_session_switching() {
	...
	echo "    - Configure $mainMod+CTRL+S to switch to Gaming Mode"
...
SUDOERS_SWITCH
	...
	local hypr_bindings_conf="${user_home}/.config/hypr/edit_here/source/keybinds.conf"

	if [[ ! -f "$hypr_bindings_conf" ]]; then
		warn "keybinds.conf not found at $hypr_bindings_conf - skipping keybind setup"
		warn "You can manually add: bindd = $mainMod CTRL, S, Gaming Mode, exec, /usr/local/bin/switch-to-gaming"
	else
		if grep -q "switch-to-gaming" "$hypr_bindings_conf" 2>/dev/null; then
			info "Gaming Mode keybind already exists in keybinds.conf"
		else
			cat >>"$hypr_bindings_conf" <<'HYPR_GAMING'

bindd = $mainMod CTRL, S, Gaming Mode, exec, /usr/local/bin/switch-to-gaming
HYPR_GAMING
			info "Added Gaming Mode keybind to keybinds.conf"
		fi
	fi
	...
	echo ""
	echo "  Usage:"
	echo "    - Press $mainMod+CTRL+S in Hyprland to switch to Gaming Mode"
	...
	echo "    - ~/.config/hypr/edit_here/source/keybinds.conf (keybind added)"
	echo ""
...
verify_installation() {
	...
	echo "  -----------------"
	local hypr_bindings="$HOME/.config/hypr/edit_here/source/keybinds.conf"
	if [[ -f "$hypr_bindings" ]]; then
		if grep -q "switch-to-gaming" "$hypr_bindings" 2>/dev/null; then
			echo "  ✓ Gaming Mode keybind ($mainMod CTRL+S) configured"
		else
			echo "  ✗ Gaming Mode keybind NOT found in keybinds.conf"
			all_ok=false
		fi
	else
		echo "  ⚠ keybinds.conf not found - keybind needs manual setup"
	fi
...
execute_setup() {
	...
	echo ""
	echo "================================================================"
	echo "  GAMING MODE INSTALLER v${GAMING_MODE_VERSION}"
	...
	if [ "$NEEDS_REBOOT" -eq 1 ]; then
		...
		echo ""
		echo "  To switch to Gaming Mode: Press $mainMod+CTRL+S"
